...
export default function ScheduleView() {
  ...
  const renderDesktopTable = (refProps, isCaptureOnly = false) => (
    <div 
      {...refProps} 
      className={`w-full ${!isCaptureOnly ? "overflow-x-auto custom-scrollbar bg-black/40 rounded-2xl p-4 shadow-2xl glass-card border-none" : "bg-[#09090b] text-white p-8 w-[1400px] block"}`}
    >
      <div className="mb-4 flex items-center justify-between pb-4 border-b border-white/10">
        ...
        {isCaptureOnly && <div className="text-white/50 text-sm">Generated by Timetable App</div>}
      </div>
      <table className="w-full text-left border-collapse table-fixed min-w-[1000px]">
        <thead>
          <tr>
            <th className={`p-3 border-b border-white/10 bg-white/5 font-semibold text-white/90 w-24 text-sm ${!isCaptureOnly ? "sticky left-0 z-20 backdrop-blur-md" : ""}`}>
              Day
            </th>
            {TIMES.map((time) => {
                const [timeVal, period] = time.split(" ");
                return (
                <th key={time} className="p-2 border-b border-white/10 bg-white/5 font-medium text-center">
              ...
              {TIMES.map((time) => {
                ...
                return (
                  <td
                    ...
                    className={`p-1 border-b border-white/5 text-center transition-all min-h-[80px] h-[100px] align-top
                  ...
                  >
                    {subjectList ? (
                      ...
                      <div className={`flex items-center justify-center w-full h-full rounded-md border border-dashed border-white/10 text-white/10 ${!isCaptureOnly && "group-hover:text-white/30 group-hover:border-white/30 group-hover:bg-white/5"} transition-all`}>
                        {!isCaptureOnly && <Plus size={16} />}
                      </div>
                    )}
                  </td>
                );
